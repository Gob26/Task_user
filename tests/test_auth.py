...
async def test_create_user(ac: AsyncClient):
    ...
    data = response.json()
    assert "id" in data                                                                #проверяем  ID
    assert data["name"] == "Вася"
    assert data["surname"] == "Пупкин"


@pytest.mark.asyncio
async def test_create_task(ac: AsyncClient):                                                                              #создаем пользователя
    ...
    assert user_id is not None, "Пользователь ID не найден"
                                                                                 #создаем задачу для этого пользователя
    ...
    assert "id" in task_data                                                            #проверяем, ID задачи
...
@pytest.mark.asyncio
async def test_read_tasks(ac: AsyncClient):                                                                                    #создаем пользователя и задачи
    user_response = await ac.post("/users/", json={"name": "Алиса", "surname": "Васелькова"})
    print(f"Создание пользователя: {user_response.status_code} - {user_response.text}")
    assert user_response.status_code == 200
    user_data = user_response.json()
    user_id = user_data.get("id")
    ...
    assert "Task 2" in task_titles, "Task 2 is missing"
                                                                               #доп проверка для каждой задачи
